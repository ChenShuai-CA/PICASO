...
> **2026-09-01 注**：本节为调研前预判。经四车 ABD 数据盘点与方案评审，"开放道路→封闭场地 UDA"已从核心叙事降级（C-NCAP 目标域为离散规程网格，经典 UDA 无可适配分布）；新核心叙事为"物理可执行生成 + 多品牌真实车辆响应闭环验证"。详见 `Stage1_研究问题凝练.md` 与《Stage2_综合差距分析》的 2026-09-01 修订块。

| 空白 | 证据 | 价值 |
|------|------|------|
| **物理约束 + 因果引导 + 生成模型的统一框架** | CounterScene 有因果无物理；Physics-informed Diffusion Mamba 有物理无因果；SaFeR 有物理可行性但基于离散 Token | 高——据当前检索三元交叉地带尚未发现直接占据者（投稿前需复核） |
| **场景生成中的多源域自适应** | 据当前检索未发现（投稿前需复核） | 中高——2026-09-01 起降级为跨品牌 surrogate 校准工具，不作核心叙事 |
| **用多品牌真实车辆响应闭环验证生成场景**（2026-09-01 起由"开放道路→封闭场地迁移"重定位） | 据当前检索未发现（投稿前需复核） | **极高**——与 ABD 实车数据护城河直接匹配 |
| **超越 ADE/FDE 的安全关键场景评估矩阵** | SaFeR/CounterScene 增加了 CR 等指标但不成体系 | 中高——形成可被后续工作引用的标准 |
| **Mamba 与因果交互的深度结合** | 据当前检索未发现（需复核） | 中——架构创新，但需与 Tamba 划清界限 |
| **CNCAP 规程约束的形式化嵌入** | 据当前检索未发现（需复核） | 高——标准-学术的桥梁，Q1 期刊重视 |
