...
未来补采路径。测试团队进一步确认这些均为 **FCW-only** 测试：只测报警，不测 AEB；通常听到报警后
由驾驶员接管。33 条 BR-zero 配对运行经人工复核全部属于这种接管，另外 153 条配对运行存在机器人
制动。因此历史 FCW→制动间隔不得进入 AEB 响应或 AEB 时刻代理校准；188 条声音上升沿本身仍可用于
FCW 报警时刻分析。
