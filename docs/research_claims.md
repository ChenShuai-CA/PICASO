...
| C3：K=1 条件排序优于固定 K=1 和脚本 | independent screen 支持，heldout 待确认 | P2.11 全新 seed76000 条件集上双分支通过预注册判据；single 差 0.044 [0.012, 0.078]，dual 差 0.037 [0.003, 0.073] |
...
3. 保持 P2.11 的主指标、无效判据和统计口径，不再根据 development 结果调参。
4. 经用户明确授权后，对 heldout 执行一次评价。
