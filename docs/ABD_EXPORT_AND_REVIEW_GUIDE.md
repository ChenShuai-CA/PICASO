...
测试团队确认这些历史 FCW 场景只测试 FCW，不测试 AEB；通常在听到报警后由驾驶员接管。因此
报警后的纵向减速不能用于 AEB 响应识别。33 条 BR-zero 配对运行已全部复核为报警后人工接管；其
余配对运行中 153 条存在机器人制动。188 条声音上升沿仍可用于 FCW 时刻分析，但 AEB 校准采用独立
的 AEB 场景人工复核池。
