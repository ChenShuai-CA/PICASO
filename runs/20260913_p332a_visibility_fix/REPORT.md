...
- teacher-forced token NLL **1.422**（均匀基线 ln128=4.85），accuracy **71.1%**；
  ...
  （平均采样熵的单样本 MC 估计，**不是**分布熵，`exp(·)` 不称有效词数）。
...
读法（与旧版相同的限定）：解码路径与教师强制域健全且优于 CV 同项；rollout 劣化
（×~4–5）来自自回归漂移+采样复合，是训练深度/暴露偏差问题，不是表示缺陷证据。
**教师强制域数字不可与 rollout 域互换，不构成"模型优于 CV"的声称**；0.7^10 式
连乘只是启发式，不是实测全对率。修正后的模型在三个行上都略优于旧模型
（1.11 vs 1.30、2.25 vs 2.54、4.03 vs 3.76 argmax-waymo），方向与"去掉泄漏路径后
表示更干净"一致，但欠训练状态下不作能力结论。
...
| **hypothesis** | 修正后模型教师强制域略优（1.11/2.25/4.03 vs 1.30/2.54/3.76）与去除泄漏路径因果相关（无对照，欠训练）；rollout 缺口将随 100-shard 正式训练量收窄 |
...
5. 加速度/jerk 几何代理被分段线性解码结构性抬高（100%）；速度违规率 0.011%；
   阈值是工程诊断不是 G1 门槛。
6. sampled_token_nll 是平均采样熵的单样本 MC 估计，不是拟合优度。
